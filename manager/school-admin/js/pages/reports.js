  ...
  window.initReports = async function(host){
    ...
    els.csv.addEventListener('click', ()=>{
      if(cachedRows.length){
        ...
        // Fallback to server CSV if no data loaded yet
        const qs = new URLSearchParams(); if(els.from.value) qs.set('from', els.from.value); if(els.to.value) qs.set('to', els.to.value);
        const url = mkApi.buildApiUrl('reports/attendance.pdf.php') + (qs.toString()?('?'+qs.toString()):'');
