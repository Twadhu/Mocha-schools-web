(function(){
  ...
  async function loadSubjects(){ const d = await safe(()=> mkApi.apiJson('api.php?action=subjects')); if(d?.ok){ state.subjects = d.subjects||[]; } }
  async function loadTeachers(){ const d = await safe(()=> mkApi.apiJson('api.php?action=teachers')); if(d?.ok){ state.teachers = d.teachers||[]; } }
  ...
  const d = await safe(()=> mkApi.apiJson('api.php?action=schedule&'+qs.toString()));
  ...
  const r = await safe(()=> mkApi.apiJson('api.php?action=schedule', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ grade, section, semester, entries }) }));
  ...
  async function hydrateBusy(teacherId, semester){ if(!teacherId) return; const key=String(teacherId); if(state.busyCache.has(key)) return; const d=await safe(()=> mkApi.apiJson(`api.php?action=teacher_schedule&teacherId=${encodeURIComponent(teacherId)}&semester=${encodeURIComponent(semester)}`)); const map={}; if(d?.ok && Array.isArray(d.items)){ d.items.forEach(it=>{ map[`${uiDay[it.day]||String(it.day).toLowerCase()}_${it.period}`]=true; }); } state.busyCache.set(key,map); }
